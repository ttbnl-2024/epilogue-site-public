...
def format_time_since(timestamp, now):
    ...
    return mark_safe(
        '<time datetime="{}" data-format="{}">{}</time>'.format(
...
def format_time(timestamp, format="DATE_TIME"):
    ...
    return mark_safe(
        f'<time datetime="{timestamp.isoformat()}" data-format="{formats.get_format(format)}">{text}</time>'
    )
...
class puzzleblock(template.Node):
    def __init__(self, parser, token):
        ...
        if len(args) not in (2, 3):
            msg = "Usage: {% puzzleblock block-name [variant] %}"
